...
class IntrospectionConfig:
    ...
    show_math_view: bool = False  # NEW: toggle equations on/off
    ...
    def clip_to_model(self, n_layer: int, n_head: int) -> "IntrospectionConfig":
        return IntrospectionConfig(
            ...
            show_math_view=self.show_math_view,
        )
...
def run_single_forward(llm: IntrospectGPT2, prompt: str, cfg: IntrospectionConfig):
    ...
    with st.expander("Step 1 & 2 – Tokenization and Numericalization", expanded=True):
        ...
        if cfg.show_math_view:
            st.markdown("#### Math view")
            st.latex(r"\text{tokens} = \mathrm{BPE}(\text{prompt})")
            st.latex(r"\text{id}_t = \mathrm{VocabLookup}(\text{token}_t)")
    ...
    with st.expander("Step 3 – Embeddings (Token IDs → dense vectors)", expanded=True):
        ...
        if token_embeds is None:
            ...
            if cfg.show_math_view:
                st.markdown("#### Math view")
            st.latex(r"\mathbf{e}_t = E[\text{id}_t], \quad \mathbf{e}_t \in \mathbb{R}^d")
            st.caption("Each token id indexes into an embedding matrix E to get a d-dimensional vector.")
    ...
    with st.expander("Step 4 – Positional encodings (add order/position to tokens)", expanded=True):
        ...
        if pos_embeds is None:
            ...
            if cfg.show_math_view:
                st.markdown("#### Math view")
            st.latex(
                r"\mathbf{h}^{(0)}_t = \mathbf{e}_t + \mathbf{p}_t"
            )
            st.caption("The input to the first transformer block is the sum of token and positional embeddings.")
    ...
    with st.expander("Step 5 – Self-Attention (Q/K/V → attention weights → context)", expanded=True):
        math_view = st.session_state.get("math_view", False)
        st.write(
        ...
        if math_view:
            st.markdown("##### Math View: Self-Attention")
            st.latex(r"Q = X W_Q,\quad K = X W_K,\quad V = X W_V")
            st.latex(r"s_{ij} = \frac{Q_i K_j^\top}{\sqrt{d_k}}")
            st.latex(r"\alpha_{ij} = \mathrm{softmax}_j(s_{ij})")
            st.latex(r"\mathrm{context}_i = \sum_j \alpha_{ij} V_j")
        ...
        if not layer_internals:
            ...
            if cfg.show_math_view and layer_internals:
             st.markdown("#### Math view")
            st.latex(r"Q = X W_Q,\quad K = X W_K,\quad V = X W_V")
            st.latex(
                r"\alpha_{i,j} = \mathrm{softmax}\!\left(\frac{Q_i K_j^\top}{\sqrt{d_k}}\right)"
            )
            st.latex(
                r"\mathbf{c}_i = \sum_j \alpha_{i,j} V_j"
            )
            st.caption(
                "Each head projects token representations X into Q, K, V, "
                "computes scaled dot-product attention, then forms a context vector as a "
    ...
    with st.expander("Step 6 – Feed-Forward Network (FC1 → activation → FC2)", expanded=True):
        ...
        if not layer_internals:
            ...
            if cfg.show_math_view and layer_ids:
             st.markdown("#### Math view")
            st.latex(
                r"\mathbf{z}_t = \mathrm{GELU}(\mathbf{x}_t W_1 + \mathbf{b}_1)"
            )
            st.latex(
                r"\mathbf{y}_t = \mathbf{z}_t W_2 + \mathbf{b}_2"
            )
            st.caption(
                "Each position t passes through a 2-layer MLP with GELU non-linearity, "
    ...
    with st.expander("Step 7 – Logits → probabilities (next-token distribution)", expanded=True):
        ...
        if cfg.show_math_view:
            st.markdown("#### Math view")
            st.latex(r"p_i = \frac{e^{\ell_i}}{\sum_j e^{\ell_j}}")
            st.caption("Softmax converts raw logits ℓ into a probability distribution over vocabulary tokens.")
    ...
    with st.expander("Step 8 – Decoding (choose next token)", expanded=True):
        ...
        if cfg.show_math_view:
            st.markdown("#### Math view")
            st.latex(r"\hat{i} = \arg\max_i \, p_i")
            st.caption(
                "Greedy decoding picks the token with maximum probability as the next token. "
                "Other strategies (top-k, nucleus sampling) sample instead of argmax."
            )
...
def run_step_by_step(llm: IntrospectGPT2, prompt: str, cfg: IntrospectionConfig, num_steps: int):
    ...
    for step in range(num_steps):
        ...
        with st.expander(
            ...
            if cfg.show_math_view:
             st.markdown("#### Math view")
            st.latex(r"\text{tokens} = \mathrm{BPE}(\text{current\_text})")
            st.latex(r"\text{id}_t = \mathrm{VocabLookup}(\text{token}_t)")
        ...
        with st.expander(
            ...
            if cfg.show_math_view and token_embeds is not None:
             st.markdown("#### Math view")
            st.latex(r"\mathbf{e}_t = E[\text{id}_t], \quad \mathbf{e}_t \in \mathbb{R}^d")
        ...
        with st.expander(
            ...
            if cfg.show_math_view and pos_embeds is not None:
             st.markdown("#### Math view")
            st.latex(r"\mathbf{h}^{(0)}_t = \mathbf{e}_t + \mathbf{p}_t")
...
# =============================================================================
# Flowchart Explorer Mode
# =============================================================================

def run_flowchart_explorer(llm: IntrospectGPT2, prompt: str, cfg: IntrospectionConfig):
    """
    Flowchart-style explorer:
      - shows an 8-node directed graph of the transformer pipeline
      - user picks a node (step) and sees a detailed explanation + visuals
    """
    cfg = cfg.clip_to_model(llm.n_layer, llm.n_head)
    info = llm.forward_with_inspection(prompt)
    ...
    st.markdown("### Flowchart Explorer – 8-Step Transformer Pipeline")
    st.write(
        "This view shows the entire pipeline as a flowchart. "
        "Select any node to inspect its internals and read an explanation."
    )
    ...
    digraph G {
        rankdir=LR;
        node [shape=box, style="rounded,filled", color="#1f77b4", fillcolor="#e6f2ff", fontsize=11];

        step1 [label="1. Tokenization\n(Text → tokens)"];
        step2 [label="2. Numericalization\n(Tokens → IDs)"];
        step3 [label="3. Embeddings\n(IDs → vectors)"];
        step4 [label="4. Positional Encodings\n(Add positions)"];
        step5 [label="5. Self-Attention\n(Q/K/V → context)"];
        step6 [label="6. Feed-Forward\n(Non-linear transform)"];
        step7 [label="7. Logits → Probs\n(Softmax)"];
        step8 [label="8. Decoding\n(Choose next token)"];

        step1 -> step2 -> step3 -> step4 -> step5 -> step6 -> step7 -> step8;
    }
    """
    st.graphviz_chart(flow_src)
    ...
    selected = st.radio(
        "Select a pipeline node to inspect:",
        step_labels,
        horizontal=True,
    )

    # Utility: quick bar helper
    def _bar(vals, labels, title, xlabel, ylabel):
        plot_bar(vals, labels, title, xlabel, ylabel)

    # ----------------- Per-step details -----------------

    # STEP 1: TOKENIZATION
    if selected.startswith("1"):
        st.subheader("Step 1 – Tokenization (Text → tokens)")
        st.write(
            "GPT-2 uses **Byte Pair Encoding (BPE)**. Your text is broken into subword tokens. "
            "This is the discrete symbolic layer the model operates on."
        )
        df = pd.DataFrame(
            {
                "Position": list(range(len(tokens))),
    ...
    # STEP 2: NUMERICALIZATION
        ...
        st.subheader("Step 2 – Numericalization (Tokens → IDs)")
        st.write(
            "Each token is mapped to an integer index into the vocabulary. "
            "The model only ever sees these integer IDs before they are embedded."
        )
        df = pd.DataFrame(
            {
                "Position": list(range(len(tokens))),
        ...
        st.dataframe(df, use_container_width=True)
        st.code(input_ids.tolist(), language="python")

    # STEP 3: EMBEDDINGS
    elif selected.startswith("3"):
        st.subheader("Step 3 – Embeddings (Token IDs → dense vectors)")
        st.write(
            "Token IDs are passed through an **embedding matrix** to produce high-dimensional "
            "vectors. Tokens with similar usage patterns tend to have similar embeddings."
        )
        if token_embeds is None:
            st.warning("No token embeddings captured.")
        else:
            emb = token_embeds[0]  # (seq, dim)
            norms = [float(v.norm().item()) for v in emb]
            st.caption("L2 norm of each token embedding (magnitude of vector):")
            _bar(
                norms,
                [f"{i}:{t}" for i, t in enumerate(tokens)],
            ...
            vals = vec[: cfg.num_dims_to_show].tolist()
            st.markdown(
                f"**Embedding for token [{idx}] = `{tokens[idx]}` – "
                f"first {cfg.num_dims_to_show} dimensions**"
            )
            _bar(
                vals,
                [str(d) for d in dims],
    ...
    # STEP 4: POSITIONAL ENCODINGS
        ...
        st.subheader("Step 4 – Positional encodings (add sequence order)")
        st.write(
            "Transformers are permutation-invariant, so we inject positional information by "
            "adding a **positional embedding** to each token embedding. This lets the model "
        ...
        if pos_embeds is None:
            ...
            pos = pos_embeds[0]  # (seq, dim)
            norms = [float(v.norm().item()) for v in pos]
            st.caption("L2 norm of positional embedding per position:")
            _bar(
                norms,
                [str(i) for i in range(len(pos))],
                "Positional embedding norms",
            ...
            vals = vec[: cfg.num_dims_to_show].tolist()
            st.markdown(
                f"**Positional embedding for position [{idx}] – "
                f"first {cfg.num_dims_to_show} dimensions**"
            )
            _bar(
                vals,
                [str(d) for d in dims],
                "First N positional dimensions",
                "Dimension index",
                "Value",
            )
    ...
    # STEP 5: SELF-ATTENTION
        ...
        st.subheader("Step 5 – Self-Attention (Q/K/V → weights → context)")
        st.write(
            "Each layer uses self-attention. For a given token, **Q** (query) compares to all "
            "**K** (keys) to produce attention weights, which are then used to average **V** "
            "(values) into a context vector."
        )

        if not layer_internals:
            st.warning("No attention internals captured.")
        else:
            layer_idx = st.slider(
                "Layer to inspect",
                0,
            ...
            if Q is None or K is None or V is None:
                ...
                scores = scores / math.sqrt(head_dim)
                weights = torch.softmax(scores[-1], dim=-1)
                ...
                matrix = weights.unsqueeze(0).numpy()
                plot_heatmap(
                    matrix,
                    x_labels=[f"{i}:{t}" for i, t in enumerate(tokens)],
                    y_labels=["Last token query"],
                    title="Attention weights (LAST token)",
                )
    ...
    # STEP 6: FEED-FORWARD
        ...
        st.subheader("Step 6 – Feed-Forward Network (FC1 → activation → FC2)")
        st.write(
            "After attention, each layer applies a small MLP to each token independently. "
            "This is where non-linear transformations happen (e.g., GELU)."
        )
        if not layer_internals:
            st.warning("No FFN internals captured.")
        else:
            ...
            if not layer_ids:
                st.warning("No FC1/FC2 data captured.")
            else:
                df = pd.DataFrame(
                    {
                        "Layer": layer_ids,
    ...
    # STEP 7: LOGITS → PROBS
        ...
        st.subheader("Step 7 – Logits → probabilities (Softmax)")
        st.write(
            "The last transformer layer outputs raw scores (**logits**) for each vocabulary token. "
    ...
    # STEP 8: DECODING
        ...
        st.subheader("Step 8 – Decoding (choose next token)")
        st.write(
            "A **decoding strategy** chooses the actual next token from the probability distribution. "
            "Here, we show the greedy choice (argmax)."
        )
        greedy_id = torch.argmax(probs, dim=-1).item()
        greedy_tok = llm.tokenizer.convert_ids_to_tokens([greedy_id])[0]
        st.markdown(
        ...
        st.write(
            "In generation, this token would be appended to the sequence and the whole "
            "pipeline repeats for the next step."
        )

# =============================================================================
# Sequence Animation Mode (per-step "MRI")
# =============================================================================

def run_sequence_animation(llm: IntrospectGPT2, prompt: str, cfg: IntrospectionConfig, num_steps: int):
    """
    Sequence Animation Mode:
    - You choose a "generation step" on a slider.
    - We simulate generation up to that step.
    - Then we show a compact 8-step view for that particular step.
    """
    cfg = cfg.clip_to_model(llm.n_layer, llm.n_head)
    math_view = st.session_state.get("math_view", False)

    st.markdown("### Sequence Animation – token-by-token brain scan")
    st.write(
        "Use the slider to pick which generation step to inspect. "
        "For that step we show the 8 conceptual stages (compressed view)."
    )
    st.write(f"**Initial prompt:** `{prompt}`")

    # 1) Choose which step to inspect (1..num_steps)
    step_to_show = st.slider(
        "Generation step to inspect",
        min_value=1,
        max_value=num_steps,
        value=1,
        step=1,
    ...
    # 2) Run generation up to that step, keeping only the last step's info
    current_text = prompt
    last_info = None
    last_probs = None
    last_tokens = None
    ...
    for s in range(step_to_show):
        ...
        greedy_id = torch.argmax(probs, dim=-1).item()
        next_tok_str = llm.tokenizer.decode([greedy_id])

        # Save state for the step we will visualize
        last_info = info
        last_probs = probs
        last_tokens = tokens
        last_greedy_id = greedy_id

        # Append for next iteration
        current_text += next_tok_str

    # Safety: if something went wrong, bail out gracefully
    if last_info is None or last_probs is None or last_tokens is None:
        st.error("Could not compute sequence animation state.")
        return

    # ----------------------------------------------------------------------
    # Now visualize the chosen step
    # ----------------------------------------------------------------------
    st.markdown("---")
    st.markdown(f"## Generation step {step_to_show}")
    st.markdown(f"**Input text at this step:** `{prompt if step_to_show == 1 else current_text[:-len(llm.tokenizer.decode([last_greedy_id]))]}`")

    input_ids = last_info["inputs"]["input_ids"][0]
    tokens = last_tokens
    probs = last_probs
    internals = last_info["internals"]
    token_embeds = internals.get("token_embeddings")
    pos_embeds = internals.get("position_embeddings")
    layer_internals = internals.get("layers", [])
    ...
    # STEP 3 – EMBEDDINGS
    # ================================================================
    with st.expander("Step 3 – Embeddings (Token IDs → dense vectors)", expanded=False):
        st.write(
            "Each token ID is mapped into a high-dimensional vector (the word embedding). "
            "We inspect norms and the first few dimensions."
        )
        if token_embeds is None:
            st.warning("No token embeddings captured.")
        else:
            ...
            plot_bar(
                norms,
                [f"{i}:{t}" for i, t in enumerate(tokens)],
                f"Embedding norms for tokens (step {step_to_show})",
                "Token index",
                "L2 norm",
            )
            ...
            idx = st.slider(
                ...
                len(tokens) - 1,
                0,
                key=f"seq_emb_idx_{step_to_show}",
            )
            vec = emb[idx]
            dims = list(range(cfg.num_dims_to_show))
            vals = vec[: cfg.num_dims_to_show].tolist()
            plot_bar(
                vals,
                [str(d) for d in dims],
                f"First N embedding dimensions (step {step_to_show})",
                "Embedding dimension index",
                "Value",
            )
    ...
    # STEP 4 – POSITIONAL ENCODINGS
    # ================================================================
    with st.expander("Step 4 – Positional encodings (add order/position)", expanded=False):
        st.write(
            "Positional embeddings let the model know which token is first, second, etc."
        )
        if pos_embeds is None:
            st.warning("No positional embeddings captured.")
        else:
            ...
            plot_bar(
                norms,
                [str(i) for i in range(len(pos))],
                f"Positional embedding norms (step {step_to_show})",
                "Position index",
                "L2 norm",
            )
    ...
    # STEP 5 – SELF-ATTENTION
    # ================================================================
    with st.expander("Step 5 – Self-Attention (Q/K/V → weights → context)", expanded=False):
        st.write(
            "Self-attention uses Q (queries), K (keys), and V (values) to mix information "
            "across positions."
        )
        if math_view:
            st.markdown("##### Math View: Self-Attention")
        ...
        if not layer_internals:
            ...
            layer_idx = st.slider(
                ...
                min(cfg.layers_to_show, len(layer_internals)) - 1,
                0,
                key=f"seq_attn_layer_{step_to_show}",
            )
            layer_data = layer_internals[layer_idx]
            Q = layer_data["Q"]
            K = layer_data["K"]
            ...
            if Q is None or K is None or V is None:
                ...
                head_idx = st.slider(
                    ...
                    min(cfg.heads_to_show, n_heads) - 1,
                    0,
                    key=f"seq_attn_head_{step_to_show}",
                )
                ...
                # SAFE: clamp top_k to length of weights
                top_k = min(cfg.top_keys_per_head, weights.shape[0])
                vals, idxs = torch.topk(weights, k=top_k)

                safe_indices = [i for i in idxs.tolist() if 0 <= i < len(tokens)]
                safe_vals = vals[: len(safe_indices)].tolist()

                df_top = pd.DataFrame(
                    {
                        "Position": safe_indices,
                        "Token": [tokens[i] for i in safe_indices],
                        "Attention weight": [float(v) for v in safe_vals],
                    }
                )
                st.dataframe(df_top, use_container_width=True)
                ...
                plot_heatmap(
                    ...
                    x_labels=[f"{i}:{t}" for i, t in enumerate(tokens)],
                    y_labels=["Last token query"],
                    title=f"Attention weights (LAST token) – step {step_to_show}",
                )

    # ================================================================
    # STEP 6 – FEED-FORWARD
    # ================================================================
    with st.expander("Step 6 – Feed-Forward Network (FC1 → activation → FC2)", expanded=False):
        st.write(
            "After attention, each layer has a small MLP processing each token independently."
        )
        if not layer_internals:
            st.warning("No FFN internals captured.")
        else:
    ...
    # STEP 7 – LOGITS → PROBABILITIES
    # ================================================================
    with st.expander("Step 7 – Logits → probabilities (next-token distribution)", expanded=False):
        st.write(
            "The final layer produces logits; softmax converts them into probabilities."
        )
        if math_view:
            st.markdown("##### Math View: Softmax")
            st.latex(r"p_i = \frac{e^{z_i}}{\sum_j e^{z_j}}")
        ...
        plot_bar(
            [float(p) for p in ps],
            [f"{tid}:{t}" for tid, t in zip(ids, texts)],
            f"Top-K next-token probabilities (step {step_to_show})",
            "Token",
            "Probability",
        )
    ...
    # STEP 8 – DECODING
    # ================================================================
    with st.expander("Step 8 – Decoding (choose next token)", expanded=False):
        greedy_id = last_greedy_id
        greedy_tok = llm.tokenizer.convert_ids_to_tokens([greedy_id])[0]
        st.markdown(
            f"**Greedy next token at step {step_to_show}:** "
            f"id={greedy_id}, token=`{greedy_tok}`, "
            f"prob={float(probs[greedy_id]):.6f}"
        )
    ...
    st.write(f"**Text after {step_to_show} generation step(s):**")
...
def main():
    ...
    cfg = IntrospectionConfig(
        ...
        # Math View toggle (global)
    math_view = st.sidebar.checkbox("Math View (show formulas)", value=False)
    st.session_state["math_view"] = math_view
    ...
    mode = st.sidebar.radio(
        "Mode",
        [
            "Single forward pass (show all 8 steps)",
            "Step-by-step generation (8 steps per token)",
            "Flowchart Explorer (clickable 8-step pipeline)",
            "Sequence Animation (token-by-token MRI)",
        ],
    )
    ...
    elif mode.startswith("Step-by-step generation"):
    ...
    elif mode.startswith("Flowchart Explorer"):
        # NO BUTTON HERE – always render explorer in this mode
        run_flowchart_explorer(llm, prompt, cfg)

    elif mode.startswith("Sequence Animation"):
        num_steps = st.number_input(
            "Number of frames / tokens to animate",
            min_value=1,
            max_value=30,
            value=8,
            step=1,
        )
        # Also no button – slider interaction must not kill the view
        run_sequence_animation(llm, prompt, cfg, num_steps=num_steps)
